...
export function RecommendedSpacesSection({ isSearchActive, filteredSpaces, filteredAdSpaces, recommendedSpaces, adSpaces, totalSearchResults, onSpaceClick, onFavoriteClick, onAdClick, EmptyState }: RecommendedSpacesSectionProps) {
  return (
            ...
            totalSearchResults > 0 ? (
              <div className="space-y-3">
                ...
                {filteredAdSpaces.map((adSpace) => (
                  <SpaceCard key={`ad-${adSpace.id}`} adSpace={adSpace} onSpaceClick={onSpaceClick} onFavoriteClick={onFavoriteClick} onAdClick={onAdClick} />
                ))}
                {/* 일반 공간 표시 */}
                {filteredSpaces.map((space) => (
                  <SpaceCard key={space.id} space={space} onSpaceClick={onSpaceClick} onFavoriteClick={onFavoriteClick} />
                ))}
              </div>
            ) : (
