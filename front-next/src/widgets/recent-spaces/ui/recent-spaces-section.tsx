...
export function RecentSpacesSection({ recentSpaces, onRecentSpaceClick, EmptyState }: RecentSpacesSectionProps) {
  return (
          ...
          {recentSpaces.length > 0 ? (
            <div className="flex space-x-3 overflow-x-auto pb-2">
              {recentSpaces.map((space) => (
                <RecentSpaceCard key={space.id} space={space} onClick={onRecentSpaceClick} />
              ))}
            </div>
          ) : (
