...
export function ManualSection() {
  ...
  const handleScroll = useCallback(() => {
    ...
    if (newIndex !== currentIndex && newIndex >= 0 && newIndex < manuals.length) {
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={handlePrevious} disabled={isScrolling}>
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={handleNext} disabled={isScrolling}>
                <ChevronRight className="h-4 w-4" />
              </Button>
            </div>
          </div>
          <CardDescription>퍼즐 스페이스 이용 방법을 쉽고 빠르게 알아보세요</CardDescription>
        </CardHeader>
        <CardContent>
          <div
          ...
          >
            {manuals.map((manual, index) => (
              <div key={manual.id} style={{ scrollSnapAlign: 'start' }} className={`transition-transform duration-200 ${isDragging ? 'scale-95' : 'scale-100'}`}>
                <ManualCard manual={manual} onClick={handleManualClick} className={`${index === currentIndex ? 'ring-2 ring-blue-400 ring-opacity-50' : ''}`} />
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-center mt-4 space-x-2">
            {manuals.map((_, index) => (
              <button key={index} className={`w-2 h-2 rounded-full transition-all duration-200 ${index === currentIndex ? 'bg-blue-500 w-6' : 'bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500'}`} onClick={() => scrollToIndex(index)} disabled={isScrolling} aria-label={`매뉴얼 ${index + 1}번으로 이동`} />
            ))}
          </div>
        </CardContent>
      </Card>
