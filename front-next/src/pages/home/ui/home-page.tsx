...
export function HomePage() {
  ...
  return (
    <>
      {/* 검색 및 필터링 컴포넌트 */}
      <SearchSpaces searchQuery={searchQuery} selectedCategory={selectedCategory} categories={safeCategories} resultCount={isSearchActive ? totalSearchResults : undefined} isSearchActive={isSearchActive} onSearchChange={handleSearchChange} onCategoryChange={handleCategoryChange} onClearSearch={handleClearSearch} onFilterClick={handleFilterClick} />

      {/* 통계 섹션 - 검색 중이 아닐 때만 표시 */}
      {!isSearchActive && <StatsSection stats={safeAppStats} />}

      {/* 추천 공간 또는 검색 결과 */}
      <RecommendedSpacesSection isSearchActive={isSearchActive} filteredSpaces={filteredSpaces} filteredAdSpaces={filteredAdSpaces} recommendedSpaces={safeRecommendedSpaces} adSpaces={safeAdSpaces} totalSearchResults={totalSearchResults} onSpaceClick={handleSpaceClick} onFavoriteClick={handleFavoriteClick} onAdClick={handleAdClick} EmptyState={EmptyState} />

      {/* 최근 본 공간 - 검색 중이 아닐 때만 표시 */}
      {!isSearchActive && <RecentSpacesSection recentSpaces={safeRecentSpaces} onRecentSpaceClick={handleRecentSpaceClick} EmptyState={EmptyState} />}

      {/* 이번 주 인기 공간 - 검색 중이 아닐 때만 표시 */}
      {!isSearchActive && <PopularSpacesSection popularSpaces={safePopularSpaces} onPopularSpaceClick={handlePopularSpaceClick} EmptyState={EmptyState} />}
