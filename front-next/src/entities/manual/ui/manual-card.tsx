...
export function ManualCard({ manual, onClick, className }: ManualCardProps) {
  ...
  return (
    <Card className={cn('flex-shrink-0 w-72 h-32 cursor-pointer hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02] overflow-hidden select-none', className)} onClick={handleClick}>
      <CardContent className="p-0 h-full">
        <div className={cn('h-full bg-gradient-to-br', manual.color, 'text-white relative')}>
          ...
          <div className="relative h-full p-4 flex flex-col justify-between">
            <div>
              <h3 className="text-sm font-semibold leading-tight mb-2 line-clamp-2">{manual.title}</h3>
            </div>

            <div className="flex items-center justify-between">
              <span className="text-xs font-medium opacity-90">{manual.linkText}</span>
