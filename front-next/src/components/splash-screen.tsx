  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 dark:from-gray-900 dark:via-indigo-900 dark:to-purple-900 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="flex flex-col items-center space-y-8 z-10">
        {/* 로고 아이콘 */}
        <div
          className={`transition-all duration-1000 ${animationStep >= 1 ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-50 translate-y-8'}`}
        >
        ...
        {/* 서비스명 */}
        <div
          className={`text-center transition-all duration-1000 delay-300 ${animationStep >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
        >
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-1 sm:mb-2 tracking-wide">
            Puzzle
            <span className="bg-gradient-to-r from-yellow-300 to-orange-400 dark:from-yellow-400 dark:to-orange-500 bg-clip-text text-transparent ml-1 sm:ml-2">
              Space
            </span>
          </h1>
          <p className="text-base sm:text-lg font-light tracking-wider text-white/80 dark:text-white/90">
            공간을 연결하다
          </p>
        </div>
        ...
        {/* 로딩 인디케이터 */}
        <div
          className={`transition-all duration-500 delay-700 ${animationStep >= 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
        >
          <div className="flex flex-col items-center space-y-4">
            ...
            {/* 로딩 텍스트 */}
            <p className="text-xs sm:text-sm text-white/60 dark:text-white/70 animate-pulse">
              공간을 불러오는 중...
            </p>
          </div>
        </div>
      ...
      <div className="absolute bottom-8 text-center">
        <p className="text-[10px] sm:text-xs text-white/40 dark:text-white/50">
          퍼즐처럼 완벽한 공간 매칭 플랫폼
        </p>
      </div>
    </div>
